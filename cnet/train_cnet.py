...
from xreal.data_utils import get_dataset_distributed_VAE, get_vindr_dataloader
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        "--base",
        nargs="*",
        metavar="xreal-concat_cond.yml",
        help="paths to base configs. Loaded from left-to-right. "
             "Parameters can be overwritten or added with command-line options of the form `--key value`.",
        default=list(['../configs/xreal-diff-t2i_cnet.yml']),
    )
    parser.add_argument(
        "-t",
        "--train",
    ...
    #     const=True,
    #     default=False,
    #     help="use organ mask instead of lesion mask",
    # )
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def __init__(self,
        ...
        if dataset_name=="vindr":
            self.train_loader_ = get_vindr_dataloader(
                dataset_path=kwargs["csv_path"],
                ...
                return_organ_mask = kwargs["return_organ_mask"],
            )

            self.val_loader_ = get_vindr_dataloader(
                dataset_path=kwargs["csv_path"],
                data_path = kwargs["data_path"],            
                batch_size=batch_size,
                img_set='val',
            ...
            self.train_loader_, self.val_loader_ = get_dataset_distributed_VAE(name="DataVAE", 
                                                                rank=0,
                                                                world_size=1, 
                                                                batch_size=self.batch_size, 
                                                                num_workers=self.num_workers,
                                                                use_rank = True if torch.cuda.device_count() > 1 else False,
                                                                controlnet=True,
                                                                return_text_labels=True,
                                                                csv_path=kwargs["csv_path"],
                                                                data_path=kwargs["data_path"],
                                                                mask_prob=kwargs["mask_prob"],
                                                                )
        # sample = next(iter(self.train_loader_))
        # mask = sample['mask']
        # self.train_loader_ = self.val_loader_ # for debugging only
        self.test_loader_ = self.val_loader_
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            with torch.no_grad(): # image logging bug here
                images = pl_module.log_images(batch, split=split,
                                              **self.log_images_kwargs)

            # append mask to images for logging
            # images['mask'] = batch['mask']
            images['control'] = (images['control']-images['control'].min()) / (images['control'].max() - images['control'].min())
            for k in images:
    ...
    def on_testing_batch_end(self, trainer, pl_module, outputs, batch, batch_idx,
                                dataloader_idx):
        if not self.disabled and pl_module.global_step > 0:
            self.log_img(pl_module, batch, batch_idx, split="val")
        if hasattr(pl_module, 'calibrate_grad_norm'):
...
if __name__ == "__main__":
    ...
    # running as `python 01_train_t2i_backbone.py`
    ...
    try:
        ...
        model.load_state_dict(load_state_dict(config.model.resume_path, location='cpu'))
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "cnet.train_cnet.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "cuda_callback": {
                "target": "cnet.train_cnet.CUDACallback"
            },
