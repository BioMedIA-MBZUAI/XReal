...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        "--base",
        nargs="*",
        metavar="xreal-ae_space_align.yml",
        help="paths to base configs. Loaded from left-to-right. "
             "Parameters can be overwritten or added with command-line options of the form `--key value`.",
        # default=list(['../configs/xreal-ae_space_align.yml']),
        default=list(['../configs/xreal-ae_bbox2mask.yml']),
    )
    parser.add_argument(
        "-t",
        "--train",
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            while not os.path.exists(self.logdir):
                ...
                time.sleep(1)
            while not os.path.exists(self.ckptdir):
                os.makedirs(self.ckptdir)
                time.sleep(1)
            while not os.path.exists(self.cfgdir):
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 rescale=False, disabled=False, log_on_batch_idx=False,
            ...
            for k in images:
                ...
                if isinstance(images[k], torch.Tensor):
                    ...
                    if self.clamp:
                        ...
                        images[k] = (images[k] - images[k].min()) / (
                                    images[k].max() - images[k].min())
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f'trianer_kwargs: {trainer_kwargs}')
